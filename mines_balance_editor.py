...
while True:
    ...
    try:
        decoded_jwt = decode(
            os.environ["jwt"].replace("Bearer ", ""),
            options={"verify_signature": False},
            algorithms=["HS256"],
        )
    except:
        new_jwt = load_jwt()
        os.environ["jwt"] = new_jwt  # Update the OS environment variable
        set_key(dotenv_file, "jwt", new_jwt)  # Update the dotenv file with the new JWT
        decoded_jwt = decode(
            os.environ["jwt"].replace("Bearer ", ""),
            options={"verify_signature": False},
            algorithms=["HS256"],
